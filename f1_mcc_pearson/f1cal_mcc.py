...
def word_level(system_word_file, gold_word_file):
    ...
    print(f'F1-Mult:{f1[0] * f1[1] * 100:.4f}')
    print(f'F1-OK:{f1[0] * 100:.4f}, F1-BAD:{f1[1]* 100:.4f}')
    print(f'MCC:{mcc[0] * 100:.4f}')
...
def sent_level(system_sent_file, gold_sent_file):
    ...
    with open(system_sent_file) as f1, open(gold_sent_file) as f2:
        ...
        print(f'Spearman: {f1.corr(f2, method="spearman") * 100:.4f}')
        print(f'Pearson:{f1.corr(f2) * 100:.4f}')
